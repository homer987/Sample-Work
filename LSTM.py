...
class LSTMmodel(tf.keras.Model):
    def __init__(self, cnn_model, num_class):
        ...
        self.lstm = tf.keras.layers.GRU(units=64, return_state=True, dropout=0.3) #### LSTM or GRU
        #self.lstm = tf.keras.layers.SimpleRNN(units=128, return_state=True, dropout=0.3)
        self.dense = tf.keras.layers.Dense(num_class, activation="softmax")

    def call (self, input):

        x = tf.keras.layers.TimeDistributed(self.cnn_model)(input)
        x, state_h1, state_c2 = self.lstm(x)
        #x, state_h1 = self.lstm(x) ##### has to be 2 variables for RNN, and 3 variabled for LSTM
        x = self.dense(x)
        output = x
...
def choose_dataset(dataset_type):
    ...
    if dataset_type == "nlp":
        ...
        #(X_train, y_train), (X_test, y_test), (_, _) = get_single_digit_dataset()
...
def main():
    ...
    # Dataset is "computer_vision" or "speech_recognition"
    ...
    #CNN_model = generic_vns_function(X_train.shape[1], layers, y_train.shape[1], layer_units, lr)

    ######## for LSTM function ########
    #LSTM_model = LSTMmodel(CNN_model, X_train.shape[1:], num_class)
    ######## for LSTM class ########
